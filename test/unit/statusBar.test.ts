...
function getStatusBarItems(): [{ text: string; tooltip: string }, { text: string; tooltip: string }] {
  const mock = window.createStatusBarItem as jest.Mock;
  const items = mock.mock.results.map((result) => result.value as { text: string; tooltip: string });
  const mainItem = items[0];
  const mcpItem = items[1];
  if (!mainItem || !mcpItem) {
    throw new Error('Expected both main and MCP status bar items to be created.');
  }
