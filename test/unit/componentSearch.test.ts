...
function createPanelMock() {
  ...
  const webview = {
    ...
    html: '',
    onDidReceiveMessage: jest.fn((callback: (message: unknown) => Promise<void>) => {
...
describe('ComponentSearchCache', () => {
  ...
  it('caches Octopart results for TTL duration', async () => {
    ...
    await cache.set('oct:stm32', [{ source: 'octopart', mpn: 'STM32', manufacturer: 'ST', description: 'MCU', offers: [], specs: [] }], 'octopart', 'STM32');
  ...
  it('returns cached result without network call', async () => {
    const context = createExtensionContextMock();
    const cache = new ComponentSearchCache(context.globalState as unknown as vscode.Memento);
    const octopart = { search: jest.fn().mockResolvedValue([{ source: 'octopart', mpn: 'STM32', manufacturer: 'ST', description: 'MCU', offers: [], specs: [] }]) };
  ...
  it('handles Octopart network error gracefully, falls back to LCSC', async () => {
    const context = createExtensionContextMock();
    const cache = new ComponentSearchCache(context.globalState as unknown as vscode.Memento);
    const octopart = { search: jest.fn().mockRejectedValue(new Error('network down')) };
    const lcscResult = [{ source: 'lcsc', mpn: 'C1234', manufacturer: 'LCSC', description: 'Capacitor', offers: [], specs: [] }];
    ...
    const secondQuickPickArgs = (vscode.window.showQuickPick as jest.Mock).mock.calls[1]?.[0] as Array<{ description: string }>;
  ...
  it('reuses the details panel and handles datasheet/copy actions', async () => {
    ...
    const service = new ComponentSearchService({ search: jest.fn() } as never, { search: jest.fn() } as never, cache);
    const result = {
    ...
    await panelMock.send({ type: 'datasheet', url: 'https://example.com/datasheet.pdf' });
  ...
  it('falls back to LCSC when Octopart returns no results', async () => {
    ...
    const lcscResult = [{ source: 'lcsc', mpn: 'C2040', manufacturer: 'LCSC', description: 'Capacitor', offers: [], specs: [] }];
    const lcsc = { search: jest.fn().mockResolvedValue(lcscResult) };
    const service = new ComponentSearchService(octopart as never, lcsc as never, cache);
  ...
  it('opens the details view when the user picks a result', async () => {
    ...
    const cache = new ComponentSearchCache(context.globalState as unknown as vscode.Memento);
    const panelMock = createPanelMock();
    const octopartResult = [{ source: 'octopart', mpn: 'STM32G0', manufacturer: 'ST', description: 'MCU', offers: [], specs: [] }];
